...
const VolForm = () => {
  ...
  const organization = "St Joseph's University";
  ...
  const handleVerifyAndClaim = (e) => {
    ...
    setTimeout(() => {
      const isRegistered = name.length > 3; // Dummy logic
        ...
        <div className="bg-white border border-slate-200 p-10 rounded-[2rem] shadow-xl shadow-slate-200/50">
          ...
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">
              Volunteer <span className="text-sky-500">Portal 2025</span>
            </h1>
            <p className="text-slate-500 text-sm mt-2">Verify your volunteer participation to claim recognition</p>
          </div>
          ...
          <form onSubmit={handleVerifyAndClaim} className="space-y-6">
            ...
            >
              {status === 'idle' && (
                <>Claim Volunteer Certificate <span className="text-xl">✨</span></>
              )}
              {status === 'verifying' && (
                <>
